// The conversation itself: the transcript, whatever the applicant is being
// asked right now, and the composer.
//
// Rendered in two places and written once. The full page
// (app/applications/new/chat/[id]/page.tsx) mounts it as a column that owns
// the viewport; the launcher drawer (app/@chat/(.)applications/new/chat/[id])
// mounts the same component inside a sheet. Only `variant` differs, and it
// only changes chrome — the derived state below (is a questionnaire open? is
// the shortlist live? is an advisor holding this?) is subtle enough that two
// copies of it would drift within a week.
...
export async function ChatThread({
  ...
  id: string;
  user: SessionUser;
  /** `page` owns the viewport; `drawer` fills whatever box the sheet gives it. */
  variant?: "page" | "drawer";
}) {
  ...
  // One sentence for what is happening right now, used as the page's
  // description and as the drawer's sub-line.
  const status =
            done
              ? canStillChat
  ...
  return (
    ...
    <div className={variant === "page" ? "flex h-[calc(100dvh-3.5rem)] flex-col" : "flex min-h-0 flex-1 flex-col"}>
      {variant === "page" ? (
        ...
        <p className="shrink-0 px-4 pb-3 text-sm text-muted-foreground text-pretty">{status}</p>
      )}

      <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain">
        <div className={variant === "page" ? "mx-auto w-full max-w-5xl px-4 py-6 sm:px-6" : "w-full px-4 py-2"}>
